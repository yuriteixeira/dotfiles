...
if [ "$POWERLINE_GIT_MODIFIED" = "" ]; then
  POWERLINE_GIT_MODIFIED="%F{black}✹%F{black}"
fi
...
if [ "$POWERLINE_SHOW_GIT_ON_RIGHT" = "" ]; then
    if [ "$POWERLINE_HIDE_GIT_PROMPT_STATUS" = "" ]; then
        POWERLINE_GIT_INFO_LEFT=" %F{yellow}%K{red}"$'\ue0b0'"%F{red}%F{black}%K{red}"$'$(git_prompt_info)$(git_prompt_status)%F{red}'
    else
        POWERLINE_GIT_INFO_LEFT=" %F{yellow}%K{red}"$'\ue0b0'"%F{red}%F{black}%K{red}"$'$(git_prompt_info)%F{red}'
    fi
